  ...
  <div>
    ...
    <form @submit.prevent="hanldeSubmit">
      ...
      <div class="mb-3">
        ...
        <div class="form-check">
          <input class="form-check-input" type="checkbox" name="tags" value="samsung" id="samsung" v-model="product.tags">
...
const validate = () => {
  ...
  if(product.value.category == null || product.value.category == '' || product.value.category == undefined){
    isValid = false;
    error.category = "Không được để trống Dah mục"
  }else{
    error.category= null;
  }
...
const hanldeSubmit = async () =>{
  ...
  try {
    await instance.post(`/products`,product.value);
